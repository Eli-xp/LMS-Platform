...
export const PublicGetCourses_API = async ({currentPageNum, limit}: {currentPageNum: number, limit:number}) => {
  console.log("PublicGetCourses_API ran");
  console.log(currentPageNum);
  const page = currentPageNum;
  const res = await centralServerAPI(`/public/courses/?page=${page}&limit=${limit}`, {
    method: "GET",
  });
